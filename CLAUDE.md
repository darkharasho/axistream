...
- Tests: `npm -w @axistream/app run test` and `npm -w @axistream/capture run test` (vitest, fork pool capped at 2). Typecheck gate: `cd packages/app && npx tsc --noEmit -p tsconfig.json`. Build gate: `npm run build` from the repo root — CI runs it, and it is the only gate that catches bundler-level breakage (e.g. renderer code importing a Node-only module); tests and `tsc` are both blind to it.
