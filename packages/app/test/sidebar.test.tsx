...
const mkState = (over: Partial<AppState> = {}): AppState => ({
  ...
  ptt: { available: true, enabled: false, active: false, error: null, mode: null, keyName: 'F18' },
  ...over,
})
...
describe('Sidebar quick toggles', () => {
  ...
  it('PTT button glows while transmitting', () => {
    render(<Sidebar active="stream" state={mkState({ ptt: { available: true, enabled: true, active: true, error: null, mode: null, keyName: 'F18' } })} onNav={() => {}} axi={mkAxi() as never} />)
  ...
  it('PTT toggle hidden when the portal is unavailable, disabled without mic, arms with mic', () => {
    const axi = mkAxi()
    const { rerender } = render(<Sidebar active="stream" state={mkState({ ptt: { available: false, enabled: false, active: false, error: null, mode: null, keyName: 'F18' } })} onNav={() => {}} axi={axi as never} />)
