...
describe('KeyPicker', () => {
  it('renders the current key chip and groups when opened', () => {
    render(<KeyPicker {...bindProps} onBind={vi.fn()} />)
  ...
  it('clicking a grid key binds it with the current modifier', () => {
    const onBind = vi.fn()
    render(<KeyPicker {...bindProps} modifier="ctrl" onBind={onBind} />)
    fireEvent.click(screen.getByRole('button', { name: 'F18' }))
  ...
  it('adding and removing a modifier rebinds', () => {
    const onBind = vi.fn()
    const { rerender } = render(<KeyPicker {...bindProps} onBind={onBind} />)
    fireEvent.click(screen.getByRole('button', { name: '+ modifier' }))
    fireEvent.click(screen.getByRole('button', { name: 'Ctrl' }))
    expect(onBind).toHaveBeenCalledWith({ key: { code: 188, name: 'F18' }, modifier: 'ctrl' })
    rerender(<KeyPicker {...bindProps} modifier="ctrl" onBind={onBind} />)
  ...
  it('warns when a letter or number is bound', () => {
    render(<KeyPicker keyName="V" keyCode={47} modifier={null} onBind={vi.fn()} />)
  ...
  it('modifier menu omits a modifier whose codes include the current keyCode', () => {
    ...
    render(<KeyPicker keyName="KEY_29" keyCode={29} modifier={null} onBind={vi.fn()} />)
    fireEvent.click(screen.getByRole('button', { name: '+ modifier' }))
