...
import { RecordMenuItems } from '../src/renderer/components/RecordButton.js'
...
describe('RecordMenuItems', () => {
  beforeEach(() => { vi.useFakeTimers() })
  afterEach(() => { vi.useRealTimers() })

  it('starts a recording when idle', () => {
    const axi = api()
    render(<RecordMenuItems recording={idle} disabled={false} axi={axi} />)

    fireEvent.click(screen.getByRole('menuitem', { name: /record/i }))
  ...
  it('shows elapsed time derived from startedAt while active', () => {
    ...
    render(<RecordMenuItems recording={active} disabled={false} axi={api()} />)
  ...
  it('stops the recording when active', () => {
    ...
    render(<RecordMenuItems recording={active} disabled={false} axi={axi} />)

    fireEvent.click(screen.getByRole('menuitem', { name: /stop recording/i }))

    expect(axi.stopRecording).toHaveBeenCalled()
  })

  it('is disabled with an explanation when an audio test is running', () => {
    render(<RecordMenuItems recording={idle} disabled axi={api()} />)

    const btn = screen.getByRole('menuitem', { name: /record/i }) as HTMLButtonElement
  ...
  it('offers to open the last recording once one has finished', () => {
    const axi = api()
    render(<RecordMenuItems recording={{ ...idle, lastPath: '/home/u/v.mp4' }} disabled={false} axi={axi} />)

    fireEvent.click(screen.getByRole('menuitem', { name: /open recording/i }))
